...
class library extends MfObject
{
    ...
    ParseXml(sXml) {
        ...
        try
        {
            ...
            this.bookField.Clear()
            
            
            iCount := XmlHelper.xpath(xml, "/library/book/Count()")
            Loop,%iCount%
            {
                sBookField := XmlHelper.xpath(xml, MfString.Format("/library/book[{0}]", A_Index))
                objBookField := new libraryBook(sBookField)
                this.bookField.Add(objBookField)
            }
    ...
    ToXml() {
        ...
        {
            smfXml := new MfString()
            smfXml.Append("<library")
            smfXml.AppendLine(">")
            for i, element in this.bookField
            {
                smfXml.AppendLine(element.ToXml())
            }
            
            smfXml.AppendLine("</library>")
            retval := smfXml.Value
        }
        catch e
        {
...
class libraryBook extends MfObject
{
    ...
    ParseXml(sXml) {
        ...
        try
        {
            ...
            this.characterField.Clear()
            
            
            ; Set Property Values
            mfsIsbnField := new MfString(XmlHelper.xDecode(xml, "/book/isbn/text()"))
            mfsIsbnField.Trim(" `t`r`n")
            this.isbnField := mfsIsbnField.Value
            
            sTitleField := XmlHelper.xpath(xml, "/book/title")
            this.titleField.ParseXml(sTitleField)
            iCount := XmlHelper.xpath(xml, "/book/author/Count()")
            Loop,%iCount%
            {
                sAuthorField := XmlHelper.xpath(xml, MfString.Format("/book/author[{0}]", A_Index))
                objAuthorField := new libraryBookAuthor(sAuthorField)
                this.authorField.Add(objAuthorField)
            }
            
            iCount := XmlHelper.xpath(xml, "/book/character/Count()")
            Loop,%iCount%
            {
                sCharacterField := XmlHelper.xpath(xml, MfString.Format("/book/character[{0}]", A_Index))
                objCharacterField := new libraryBookCharacter(sCharacterField)
                this.characterField.Add(objCharacterField)
            }
            
            mfsAvailableFieldSpecified := new MfString(XmlHelper.xpath(xml, "/book/@available/text()"))
            mfsAvailableFieldSpecified.Trim(" `t`r`n")
            this.availableFieldSpecified := (mfsAvailableFieldSpecified.Length > 0)
            
            ; Set Attribute Values
            strIdField := XmlHelper.xDecode(xml, "/book/@id/text()")
            strIdField := Trim(strIdField, " `t`r`n")
            this.idField := strIdField
            strAvailableField := XmlHelper.xpath(xml, "/book/@available/text()")
            strAvailableField := Trim(strAvailableField, " `t`r`n")
            if (!MfNull.IsNull(strAvailableField))
            {
                bAvailableField := new MfBool(MfBool.False)
                if(MfBool.TryParse(bAvailableField, strAvailableField))
                {
                    this.availableField := bAvailableField.Value
                }
            }
            else
            {
    ...
    ToXml() {
        ...
        try
        {
            smfXml := new MfString()
            smfXml.Append("<book")
            smfXml.Append(MfString.Format(" id={0}{1}{0}", """", XmlHelper.Encode(this.idField)))
            
            mfbAvailableField := new MfBool(this.availableField)
            smfXml.Append(MfString.Format(" available={0}{1}{0}", """", mfbAvailableField.ToString()))
            
            smfXml.AppendLine(">")
            ; Read Property Values
            smfXml.Append("<isbn>")
            smfXml.Append(XmlHelper.Encode(this.isbnField))
            smfXml.AppendLine("</isbn>")
            
            smfXml.Append("<title>")
            smfXml.Append(this.titleField.ToXml())
            smfXml.AppendLine("</title>")
            
            for i, element in this.authorField
            {
                smfXml.AppendLine(element.ToXml())
            }
            
            for i, element in this.characterField
            {
                smfXml.AppendLine(element.ToXml())
            }
            
            smfXml.AppendLine("</book>")
            retval := smfXml.Value
        }
        catch e
        {
...
class libraryBookTitle extends MfObject
{
    ...
    ParseXml(sXml) {
        ...
        try
        {
            ...
            ; Set Property Values
            mfsValueField := new MfString(XmlHelper.xDecode(xml, "/title/text()"))
            mfsValueField.Trim(" `t`r`n")
            this.valueField := mfsValueField.Value
            
            ; Set Attribute Value
            strLangField := XmlHelper.xDecode(xml, "/title/@lang/text()")
            strLangField := Trim(strLangField, " `t`r`n")
            this.langField := strLangField
        }
        catch e
        {
            ex := new MfException(MfString.Format(MfEnvironment.Instance.GetResourceString("Exception_Error"), A_ThisFunc), e)
    ...
    ToXml() {
        ...
        try
        {
            smfXml := new MfString()
            smfXml.Append("<title")
            smfXml.Append(MfString.Format(" lang={0}{1}{0}", """", XmlHelper.Encode(this.langField)))
...
class libraryBookAuthor extends MfObject
{
    ...
    ParseXml(sXml) {
        ...
        try
        {
            ...
            ; Set Property Values
            sNameField := XmlHelper.xpath(xml, "/author/name")
            this.nameField.ParseXml(sNameField)
            mfsBornField := new MfString(XmlHelper.xDecode(xml, "/author/born/text()"))
            mfsBornField.Trim(" `t`r`n")
            this.bornField := mfsBornField.Value
            
            mfsDeadField := new MfString(XmlHelper.xDecode(xml, "/author/dead/text()"))
            mfsDeadField.Trim(" `t`r`n")
            this.deadField := mfsDeadField.Value
            
            ; Set Attribute Value
            strIdField := XmlHelper.xDecode(xml, "/author/@id/text()")
            strIdField := Trim(strIdField, " `t`r`n")
            this.idField := strIdField
        }
        catch e
    ...
    ToXml() {
        ...
        {
            smfXml := new MfString()
            smfXml.Append("<author")
            smfXml.Append(MfString.Format(" id={0}{1}{0}", """", XmlHelper.Encode(this.idField)))
            
            smfXml.AppendLine(">")
            ; Read Property Values
            smfXml.Append("<name>")
            smfXml.Append(this.nameField.ToXml())
            smfXml.AppendLine("</name>")
            
            smfXml.Append("<born>")
            smfXml.Append(XmlHelper.Encode(this.bornField))
            smfXml.AppendLine("</born>")
            
            smfXml.Append("<dead>")
            smfXml.Append(XmlHelper.Encode(this.deadField))
            smfXml.AppendLine("</dead>")
            
            smfXml.AppendLine("</author>")
            retval := smfXml.Value
        }
        catch e
        {
...
class libraryBookAuthorName extends MfObject
{
    ...
    ParseXml(sXml) {
        ...
        try
        {
            ...
            ; Set Property Values
            mfsFirstField := new MfString(XmlHelper.xDecode(xml, "/name/first/text()"))
            mfsFirstField.Trim(" `t`r`n")
            this.firstField := mfsFirstField.Value
            
            mfsMiddleField := new MfString(XmlHelper.xDecode(xml, "/name/middle/text()"))
            mfsMiddleField.Trim(" `t`r`n")
            this.middleField := mfsMiddleField.Value
            
            mfsLastField := new MfString(XmlHelper.xDecode(xml, "/name/last/text()"))
            mfsLastField.Trim(" `t`r`n")
            this.lastField := mfsLastField.Value
    ...
    ToXml() {
        ...
        try
        {
            ...
            ; Read Property Values
            smfXml.Append("<first>")
            smfXml.Append(XmlHelper.Encode(this.firstField))
            smfXml.AppendLine("</first>")
            
            smfXml.Append("<middle>")
            smfXml.Append(XmlHelper.Encode(this.middleField))
            smfXml.AppendLine("</middle>")
            
            smfXml.Append("<last>")
            smfXml.Append(XmlHelper.Encode(this.lastField))
            smfXml.AppendLine("</last>")
...
class libraryBookCharacter extends MfObject
{
    ...
    ParseXml(sXml) {
        ...
        try
        {
            ...
            ; Set Property Values
            mfsNameField := new MfString(XmlHelper.xDecode(xml, "/character/name/text()"))
            mfsNameField.Trim(" `t`r`n")
            this.nameField := mfsNameField.Value
            
            mfsBornField := new MfString(XmlHelper.xDecode(xml, "/character/born/text()"))
            mfsBornField.Trim(" `t`r`n")
            this.bornField := mfsBornField.Value
            
            mfsQualificationField := new MfString(XmlHelper.xDecode(xml, "/character/qualification/text()"))
            mfsQualificationField.Trim(" `t`r`n")
            this.qualificationField := mfsQualificationField.Value
            
            ; Set Attribute Value
            strIdField := XmlHelper.xDecode(xml, "/character/@id/text()")
            strIdField := Trim(strIdField, " `t`r`n")
            this.idField := strIdField
        }
        catch e
    ...
    ToXml() {
        ...
        try
        {
            ...
            smfXml.Append("<qualification>")
            smfXml.Append(XmlHelper.Encode(this.qualificationField))
            smfXml.AppendLine("</qualification>")
            
            smfXml.AppendLine("</character>")
            retval := smfXml.Value
        }
        catch e
        {
